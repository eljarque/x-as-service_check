...
function tone(questionId: string, v: AnswerValue): string {
  const q = questionById(questionId);
  return TONE[q ? classifyAnswer(q, v) : 'na'];
}
            ...
            {divs.map((d) => (
              <li key={d.questionId} className="rounded-lg border border-slate-100 bg-slate-50 p-3">
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  <span className={`rounded-md px-2 py-1 font-medium ${tone(d.questionId, d.provider)}`}>
